...
function loadPermissionById(){
   var fid=${param.fid};
   if(fid=="" || fid="null")
     return ;
   
   $.ajax({   
        url:'<%=basePath%>/admin/getResource.do',   
        type:'post',   
        data:'fid='+fid,   
        async : true, //默认为true 异步   
        error:function(){   
        ...
        success:function(data){
          if (data.status == 'OK') {
             var item=data.data;
             $("#pid").val(item.pid);
             $("#functionName").val(item.functionName);
             $("#functionUrl").val(item.functionUrl);
             $("#functionDesc").val(item.functionDesc);
             $("#postion").val(item.postion);
             $("#fid").val(item.fid);  
